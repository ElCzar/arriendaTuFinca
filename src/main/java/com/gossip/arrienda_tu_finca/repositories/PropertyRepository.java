...
public interface PropertyRepository extends JpaRepository<Property, Long> {
    ...
    @Query("SELECT new com.gossip.arrienda_tu_finca.dto.PropertyDTO(p.id, p.name, p.description, p.municipality, p.typeOfEntrance, p.peopleNumber, p.address, p.isAvailable, p.pricePerNight, p.amountOfRooms, p.amountOfBathrooms, p.isPetFriendly, p.hasPool, p.hasGril, p.owner.email) FROM Property p WHERE p.municipality = :municipality")
    List<PropertyDTO> findAllPropertyDTOByMunicipality(@Param("municipality") String municipality);

    // Busqueda de una propiedad por su nombre
    @Query("SELECT new com.gossip.arrienda_tu_finca.dto.PropertyDTO(p.id, p.name, p.description, p.municipality, p.typeOfEntrance, p.peopleNumber, p.address, p.isAvailable, p.pricePerNight, p.amountOfRooms, p.amountOfBathrooms, p.isPetFriendly, p.hasPool, p.hasGril, p.owner.email) FROM Property p WHERE p.name = :name")
    PropertyDTO findPropertyDTOByName(@Param("name") String name);
    
    // Busqueda de una propiedad por su municipio
    @Query("SELECT new com.gossip.arrienda_tu_finca.dto.PropertyDTO(p.id, p.name, p.description, p.municipality, p.typeOfEntrance, p.peopleNumber, p.address, p.isAvailable, p.pricePerNight, p.amountOfRooms, p.amountOfBathrooms, p.isPetFriendly, p.hasPool, p.hasGril, p.owner.email) FROM Property p WHERE p.municipality = :municipality")
    PropertyDTO findPropertyDTOByMunicipality(@Param("municipality") String municipality);

    // Busqueda de una propiedad por su cantidad de personas
    @Query("SELECT new com.gossip.arrienda_tu_finca.dto.PropertyDTO(p.id, p.name, p.description, p.municipality, p.typeOfEntrance, p.peopleNumber, p.address, p.isAvailable, p.pricePerNight, p.amountOfRooms, p.amountOfBathrooms, p.isPetFriendly, p.hasPool, p.hasGril, p.owner.email) FROM Property p WHERE p.peopleNumber = :peopleNumber")
    PropertyDTO findPropertyDTOByPeopleNumber(@Param("peopleNumber") Integer peopleNumber);
